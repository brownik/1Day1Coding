...
링크 - [바탕화면 정리](https://school.programmers.co.kr/learn/courses/30/lessons/161990)
...
class Solution {
    fun solution(wallpaper: Array<String>): IntArray {
        var answer = List(4) { -1 }.toMutableList()
        wallpaper.mapIndexed { i, d ->
            if (d.contains("#")) {
                val min = d.indexOfFirst { it == '#' }
                val max = d.indexOfLast { it == '#' }
                if (answer[0] == -1 || i < answer[0]) answer[0] = i
                if (answer[1] == -1 || min < answer[1]) answer[1] = min
                if (answer[2] == -1 || i >= answer[2]) answer[2] = i + 1
                if (answer[3] == -1 || max >= answer[3]) answer[3] = max + 1
            }
        }
