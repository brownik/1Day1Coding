...
링크 - [수식 최대화](https://school.programmers.co.kr/learn/courses/30/lessons/67257)
...
class Solution {
    fun solution(expression: String): Long {
        ...
        val baseOperator = listOf('+', '-', '*')
        val useOperator = mutableSetOf<String>()
        var check = ""

        expression.forEach {
            if (!baseOperator.contains(it)) { check += it }
            else { list.add(check); check = ""; list.add(it.toString()); useOperator.add(it.toString()) }
        }
        list.add(check)

        return permutation(useOperator.toList()).map { order ->
            val form = list.toCollection(mutableListOf())
            order.forEach { operator ->
                while (form.contains(operator)) {
                    val index = form.indexOf(operator)
                    val number = calculate(form[index - 1], form[index], form[index + 1])
                    form[index - 1] = number.toString()
                    form.removeAt(index + 1)
                    form.removeAt(index)
                }
            }
            abs(form.first().toLong())
        }.maxOf { it }
    }

    private fun calculate(first: String, second: String, third: String): Long{
        return when(second) {
