...
class Solution {
    fun solution(progresses: IntArray, speeds: IntArray): IntArray {
        ...
        for (i in progresses.indices) {
            ...
            val remainTime = if (remain % speeds[i] == 0) remain / speeds[i] else remain / speeds[i] + 1
            if (releaseRemain == 0) { releaseRemain = remainTime; continue }
            if (remainTime <= releaseRemain) {
                release += 1
            } else {
                releaseRemain = remainTime
                answer.add(release)
                release = 1
            }
        }
