...
링크 - [덧칠하기](https://school.programmers.co.kr/learn/courses/30/lessons/161989#qna)
...
class Solution {
    fun solution(n: Int, m: Int, section: IntArray): Int {
        var answer = 0
        var wall = List(n) { if (section.contains(it + 1)) 0 else 1 }.joinToString("")
        while (wall.contains("0")) {
            val index = wall.indexOfFirst { it == '0' }
            val last = if (index + m < wall.length) index + m else wall.length
            wall = wall.replaceFirst(wall.substring(index until last), "")
            answer += 1
        }
        return answer
    }
...
첫 번째로 풀었던 풀이.. 작성하다 보니 시간도 오래 걸리고 벽의 길이가 필요하나 싶었다.

주어진 section(다시 칠해야 하는 벽의 번호)만 사용해서 풀 수 있을 것 같았다.
...
원래는 section에 1이 있으면 1을 추가해주고 아니면 그대로 진행하려는 코드를 작성했는데 복잡해지는 것 같아 다른 사람의 풀이를 보니 파이썬으로 똑같은 코드로 풀었더라는..

풀이 시간이 1000ms 단위에서 0.1ms 단위로 줄었다.. 대박
