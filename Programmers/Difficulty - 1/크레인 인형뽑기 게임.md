...
Difficulty: 1
Last Updated: 2023년 6월 7일 오후 3:54
Resolved: true

링크 - [크레인 인형뽑기 게임](https://school.programmers.co.kr/learn/courses/30/lessons/64061)
...
class Solution {
    fun solution(board: Array<IntArray>, moves: IntArray): Int {
        ...
        val box = MutableList(board.size) { i ->
            List(board.first().size) { j ->
                board[j][i]
            }.filter { it != 0 }
        }
        ...
        moves.forEach { num ->
            val point = num - 1
            val catch = box[point].firstOrNull()
            catch?.let {
                if (list.lastOrNull() == it) {
                    list.removeLast()
                    answer += 2
                } else {
                    list.add(it)
                }
                box[point] = box[point].drop(1)
            }
        }
