...
Difficulty: 0
Last Updated: 2023년 5월 11일 오전 11:13
Resolved: true

링크 - [전국 대회 선발 고사](https://school.programmers.co.kr/learn/courses/30/lessons/181851#)
...
class Solution {
    fun solution(rank: IntArray, attendance: BooleanArray): Int {
        val candidate = attendance.mapIndexed { i, b ->
            Triple(i, rank[i], b)
        }.filter { it.third }.sortedBy { it.second }
        return 10000 * candidate[0].first + 100 * candidate[1].first + candidate[2].first
    }
}
```
...
class Solution {
    fun solution(rank: IntArray, attendance: BooleanArray): Int {
        ...
        rank.forEachIndexed { i, r ->
            if (attendance[i]) {
                when {
                    top.size < 3 -> top.add(r)
                    top.last() > r -> top[2] = r
                }
            }
            top.sort()
        }
        val num = top.map { rank.indexOf(it) }
